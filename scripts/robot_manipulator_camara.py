...
def robot_manipulator_camara():

	
	rospy.init_node('robot_manipulator_camara', anonymous=True)  #inicializamos el nodo
	rospy.Subscriber('robot_manipulator_ping_pong', String, callback)
	
	rate = rospy.Rate(10) #definimos la frecuencia de actualización

	
	
	#print("Esperando color:")
	#X=input()
	#print(X)
	rospy.spin()
